...
def encode_shot(obs, src_id, target_id, ships_sent):
    """Trả về vector 24 float hoặc None"""
    ...
    for p in planets:
        pid = int(p[0])
        p_dict[pid] = (int(p[1]), float(p[2]), float(p[3]), float(p[4]), int(p[5]), int(p[6]))

    if src_id not in p_dict or target_id not in p_dict:
        return None

    src_owner, sx, sy, sr, ss, sp = p_dict[src_id]
    tgt_owner, tx, ty, tr, ts, tp = p_dict[target_id]

    my_ships_total = sum(p[5] for p in planets if int(p[1]) == player)
    enemy_ships_total = sum(p[5] for p in planets if int(p[1]) >= 0 and int(p[1]) != player)
    my_planets_cnt = sum(1 for p in planets if int(p[1]) == player)
    enemy_planets_cnt = sum(1 for p in planets if int(p[1]) >= 0 and int(p[1]) != player)

    dx = tx - sx
    dy = ty - sy
    dist_raw = math.hypot(dx, dy)
    dist_eff = max(0.0, dist_raw - sr - tr)

    if ships_sent <= 0:
        speed = 1.0
    else:
        log_ratio = math.log(max(ships_sent, 1)) / math.log(1000.0)
        log_ratio = max(0.0, min(1.0, log_ratio))
        speed = 1.0 + (MAX_SPEED_VAL - 1.0) * (log_ratio ** 1.5)
    eta = dist_eff / max(speed, 0.5)

    own_self = 1.0 if tgt_owner == player else 0.0
    own_neutral = 1.0 if tgt_owner == -1 else 0.0
    own_enemy = 1.0 if (tgt_owner >= 0 and tgt_owner != player) else 0.0
    ...
    feat = np.array([
        ...
        own_neutral,
        own_enemy,
        ships_sent / MAX_SHIPS_VAL,
        ship_frac,
        dist_eff / BOARD_SIZE,
        eta / 60.0,
        speed / MAX_SPEED_VAL,
        ally_n / 10.0,
        ally_s / MAX_SHIPS_VAL,
        enemy_n / 10.0,
        enemy_s / MAX_SHIPS_VAL,
        step / 500.0,
        my_ships_total / (MAX_SHIPS_VAL * MAX_PLANETS_VAL),
        enemy_ships_total / (MAX_SHIPS_VAL * MAX_PLANETS_VAL),
        (my_ships_total - enemy_ships_total) / (MAX_SHIPS_VAL * MAX_PLANETS_VAL),
        my_planets_cnt / MAX_PLANETS_VAL,
        enemy_planets_cnt / MAX_PLANETS_VAL
    ], dtype=np.float32)
    return feat

def find_target_by_ray(src_xy, angle, planets, ray_horizon=200.0, perp_margin=1.0):
    sx, sy = src_xy
    fx = math.cos(angle)
    fy = math.sin(angle)
    best_pid = -1
...
class DiffusionPolicy(torch.nn.Module):
    def __init__(self, state_dim=32, action_dim=8, hidden_dim=256):
        ...
        self.net = torch.nn.Sequential(
            torch.nn.Linear(state_dim + action_dim + hidden_dim, hidden_dim),
            torch.nn.ReLU(),
...
def get_weights_path():
    candidates = [
        Path("/kaggle_simulations/agent/diffusion_policy.pth"),
        Path.cwd() / "diffusion_policy.pth",
        Path("diffusion_policy.pth")
    ]
    try:
        candidates.insert(0, Path(__file__).resolve().parent / "diffusion_policy.pth")
    except NameError:
        pass
    for p in candidates:
        if p.exists():
            return str(p)
    return "diffusion_policy.pth"
...
def sample_action(state, num_steps=50):
    with torch.no_grad():
        ...
        for step in range(num_steps):
            ...
            t_tensor = torch.full((1,), t_val, dtype=torch.float32)
            noise_pred = model(state_t, action, t_tensor)
            action = action - dt * noise_pred
        probs = torch.softmax(action, dim=-1).numpy()[0]
...
def heuristic_agent_internal(obs, config=None,return_phase = True, forced_phase = None):
    ...
    if not world.my_planets:
        return ([], phase) if return_phase else []
...
def get_cql_path():
    candidates = [
        Path("/kaggle_simulations/agent/cql_policy.pt"),
        Path.cwd() / "cql_policy.pt",
        Path("cql_policy.pt")
    ]
    try:
        candidates.insert(0, Path(__file__).resolve().parent / "cql_policy.pt")
    except NameError:
        pass
    for p in candidates:
        if p.exists():
...
if cql_path_str:
    try:
        ...
        print(f"Failed to load CQL policy: {e}")
        cql = None
else:
    print("cql_policy.pt not found, CQL disabled")


# Load validator weights
class NumpyValidator:
    def __init__(self, w):
        self.w0, self.b0 = w['w0'], w['b0']
        self.w2, self.b2 = w['w2'], w['b2']
        self.w4, self.b4 = w['w4'], w['b4']
    def proba(self, x):
        h = np.maximum(0, x @ self.w0.T + self.b0)
        h = np.maximum(0, h @ self.w2.T + self.b2)
        logit = h @ self.w4.T + self.b4
        return 1 / (1 + np.exp(-logit))

def get_val_path():
    candidates = [
        Path("/kaggle_simulations/agent/validator_weights.npz"),
        Path.cwd() / "validator_weights.npz",
...
val_path_str = get_val_path()
if val_path_str:
    try:
        val_weights = np.load(val_path_str)
        _VALIDATOR = NumpyValidator(val_weights)
        print(f"Validator loaded successfully from {val_path_str}")
    except Exception as e:
    ...
    if _VALIDATOR is not None and moves:
        ...
        for mv in moves:
            ...
            if tgt_id < 0 or tgt_id == src_id:
                filtered.append(mv)
                continue
            if owner_by_id.get(tgt_id, -2) == player:
                filtered.append(mv)
                continue
            ...
            if feat is None:
                filtered.append(mv)
                continue
            prob = _VALIDATOR.proba(feat.reshape(1, -1))[0]
